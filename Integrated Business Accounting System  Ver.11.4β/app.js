...
function repositionKeypadForIPhoneSafari() {
  ...
  const keypadWidth = Math.min(
    isLandscape ? 300 : 360,
    viewportWidth - (isLandscape ? 8 : 16)
  );
  ...
  if (isLandscape) {
    const right = Math.max(
      gap,
      Math.round(window.innerWidth - viewportRight + gap)
    );
    ...
    keypad.style.setProperty('transform-origin', 'bottom center', 'important');
  }
}
...
// === BOTTLE_BASE から自動入力ルールを生成 ===
const bottleRules = Object.freeze(
  Object.fromEntries(
    Object.entries(BOTTLE_BASE).map(([name, amt]) => [
      name,
      {
        split: 1,
        qty: 1,
