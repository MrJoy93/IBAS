...
function addBottleSubrow(mainTr) {
  ...
  const start = cols - (hasLeaveCol ? 5 : 4);
  ...
  html += `
  <td class="btl-detail">
    <div class="btl-detail-inner">
      <select class="bottleDetails">${getBottleOptionsHTML()}</select>
      <button type="button" class="bottle-hierarchy-btn">選択</button>
    </div>
...
`;
  ...
  const selectEl = tr.querySelector('.bottleDetails');
  if (selectEl) {
    // 初期状態を空に
    ...
    selectEl.addEventListener('change', () => {
      ...
      const row = selectEl.closest('.btl-subrow');
      if (row) {
        updateBottleAmountForRow(row);
  ...
  mainTr.dataset.bottleCount = String(subs.length);
...
function removeBottleSubrow(mainTr, targetSub = null) {
  ...
  if (minusBtn) {
    ...
    if (typeof saveBulkGridState === 'function') saveBulkGridState();
...
['f','f2','jounai','honshiri','douhan','eda','help','set40','set20','vip','a','b','c','d','e']
  .forEach(k => {
    ...
    html += `<td><input data-k="${k}" class="bulk-num bulk-custom-keypad-target" inputmode="numeric" placeholder="${ph}"></td>`;
  });
...
// 他は従来どおり数値
['jounai','honshiri','douhan','eda','help','set40','set20','vip','a','b','c','d','e']
  .forEach(k => {
  ...
  const amount = parseInt((n.querySelector('.bottleAmount')?.value||'0').replace(/,/g,'')) || 0;
  ...
  for(const r of rows){
    ...
    // ★ 残り
    ['jounai','honshiri','douhan','eda','help','set40','set20','vip','a','b','c','d','e']
    .forEach(id => setValue(id, r.nums[id] || ''));
  ...
  ensureBottleOptionExists(b.detail);
        ...
        // 旧: if(typeof calculate==='function') calculate();
...
function rememberBottleSelectionsFromMainRow(mainRow) {
  ...
  const subs = typeof getBottleSubrows === 'function'
    ? getBottleSubrows(mainRow)
    : Array.from(mainRow.parentNode.querySelectorAll(`tr.btl-subrow[data-parent-id="${mainRow.dataset.rowId}"]`));
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if (mainTr) {
    ...
    if (typeof window.saveBulkGridState === 'function') {
      window.saveBulkGridState();
    }
  } else {
 ...
 function saveBulkGridState() {
  ...
  const rows = mains.map(tr => {
    ...
    tr.querySelectorAll('[data-k]').forEach(el => {
      ...
      if (el.type === 'checkbox') row.nums[k] = !!el.checked;
      else row.nums[k] = el.value || '';
    });

    let n = tr.nextElementSibling;
    while (n && n.classList.contains('btl-subrow')) {
      row.bottles.push({
      detail: getSelectedDetail(n) || '',
...
window.saveBulkGridState = saveBulkGridState;
  ...
  data.forEach((r, i) => {
    ...
    // ★ data-k要素を一度だけ取得して辞書化（高速化の肝）
    const kmap = {};
    tr.querySelectorAll('[data-k]').forEach(el => { kmap[el.dataset.k] = el; });
    ...
    for (const k in nums) {
      ...
      if (el.type === 'checkbox') {
        el.checked = !!nums[k];             // ★ checkbox復元
      } else {
        el.value = norm(nums[k]);           // ★ 0は空欄扱い
      }
    }
    ...
    if (r.bottles?.length) {
      ...
      r.bottles.forEach(b => {
        ...
        if (last?.classList.contains('btl-subrow')) {
          setBottleDetailValue(last.querySelector('.bottleDetails'), b.detail || '');
          last.querySelector('.bottleDetails')?.dispatchEvent(new Event('change', { bubbles: true }));
          last.querySelector('.splitCount').value = norm(b.split);
          last.querySelector('.bottleQuantity').value = norm(b.qty);
          last.querySelector('.bottleAmount').value = norm(b.amount);
        }
      });
    }
  });
